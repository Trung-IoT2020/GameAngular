...
@Component({
  selector: 'app-fish-eat-bubbles',
  templateUrl: './fish-eat-bubbles.component.html',
  styleUrls: ['./fish-eat-bubbles.component.scss']
})


export class FishEatBubblesComponent implements OnInit, AfterViewInit {
  ...
  contructData(): any {
    ...
    this.player = new Player(
      ...
      50, 0, 0, 0, 0, 498, 327,
      this.mouse.x, this.mouse.y, this.gameFrame,
      this.mouse.click, this.playerLeft, this.playerRight, this.ctx
    );
    ...
    this.BG = {
      ...
      x1: 0,
      y1: 0,
      x2: this.canvasRef.nativeElement.width,
      y: 0,
      y2: 0,
  ...
  ngAfterViewInit(): void {
    ...
    this.canvasRef.nativeElement.addEventListener('mousedown', (event: any) => {
      ...
      this.player.mouseX = event.x - canvarPosition.left;
      this.player.mouseY = event.y - canvarPosition.top;
      this.player.MouseClick = true;
  ...
  updatePlayer(): any {
    ...
    if (this.player.y != this.mouse.y) {
      this.player.y -= dy / 100;
    }
    // console.log(this.player.x, this.player.y, this.mouse.x, this.mouse.y);
    if (this.gameFrame % 5 === 0) {
      this.player.frame++;
      if (this.player.frame >= 12) {
        this.player.frame = 0;
      }
      if (this.player.frame === 3 || this.player.frame === 7 || this.player.frame === 0 || this.player.frame === 11) {
